...
const float EPSILON2   = EPSILON * 0.9; // factor >= 1 causes artifacts
                          ...
                          inout bool ray_starts_inside_sphere)
{
    ...
    for(int i = sphere_start_index; i < sphere_stop_index; i++) {
        ...
        if(i == ignore_obj_index && abs(ray_tracer_sphere_eta[local_index]) == BIG_NUMBER) { // prevent self-intersection for opaque materials
            continue;
        }
        vec3 _surface_normal;
    ...
    for(int i = plane_start_index; i < plane_stop_index; i++) {
        ...
        if(i == ignore_obj_index && abs(ray_tracer_plane_eta[local_index]) == BIG_NUMBER) { // prevent self-intersection for opaque materials
            continue;
        }
    ...
    for(int i = box_start_index; i < box_stop_index; i++) {
        ...
        if(i == ignore_obj_index && abs(ray_tracer_box_eta[local_index]) == BIG_NUMBER) { // prevent self-intersection for opaque materials
            continue;
        }
        vec3 _surface_point;
        vec3 _surface_normal;
...
float ray_sphere_next_ray(in    vec3  ray_origin,
                          in    vec3  ray_dir,
                          in    float dist,
                          ...
                          inout vec3  next_ray)
{
    ...
    if(abs(sphere_eta) == BIG_NUMBER) { // opaque material
        _next_ray = reflect(ray_dir, surface_normal);
    } else { // transparent material
        if(ray_starts_inside_sphere) { // ray starts inside sphere
            _next_ray = refract(ray_dir, -surface_normal, 1.0f / sphere_eta);
            if(length(_next_ray) == 0) { // refract fails -- total internal reflection
                _next_ray = reflect(ray_dir, -surface_normal);
                total_internal_reflection = true;
            }
        } else { // ray starts outside sphere
            _next_ray = refract(ray_dir, surface_normal, sphere_eta);
        }
    }
    if(sphere_eta == -BIG_NUMBER) { // diffuse material
        next_ray = normalize(_next_ray + get_random_offset() * sphere_diffuse_fuzz);
    } else {
        next_ray = _next_ray;
    }
    if(abs(sphere_eta) == BIG_NUMBER) { // opaque material
...
float ray_plane_next_ray(in    vec3  ray_origin,
                         in    vec3  ray_dir,
                         in    float dist,
                         in    vec3  plane_point,
                         in    vec3  plane_normal,
                         ...
                         inout vec3  next_ray)
{
    ...
    if(abs(plane_eta) == BIG_NUMBER) { // opaque material
        _next_ray = reflect(ray_dir, plane_normal);
    } else { // transparent material
        if(dot(ray_origin, plane_normal) < dot(plane_point, plane_normal)) { // ray starts inside plane
            _next_ray = refract(ray_dir, -plane_normal, 1.0f / plane_eta);
            if(length(_next_ray) == 0) { // refract fails -- total internal reflection
                _next_ray = reflect(ray_dir, -plane_normal);
                total_internal_reflection = true;
            }
        } else { // ray starts outside plane
            _next_ray = refract(ray_dir, plane_normal, plane_eta);
        }
    }
    if(plane_eta == -BIG_NUMBER) { // diffuse material
        next_ray = normalize(_next_ray + get_random_offset() * plane_diffuse_fuzz);
    } else {
        next_ray = _next_ray;
    }
    if(abs(plane_eta) == BIG_NUMBER) { // opaque material
...
void main()
{
    ...
    for(int iter = 0; iter <= last_ray_tracer_bounce_index; iter++) {
        ray_dir_stack[iter] = ray_dir;
        vec3 surface_point;
        vec3 surface_normal;
        bool ray_starts_inside_sphere = false;
        dist_stack[iter] = ray_world_intersect(ray_origin,
                                               ray_dir,
                                               ignore_obj_index,
                                               false,
                                               obj_index,
        ...
        if(obj_index >= sphere_start_index && obj_index < sphere_stop_index) { // ray hits sphere
            ...
            dist_stack[iter] = ray_sphere_next_ray(ray_origin,
                                                   ray_dir,
                                                   dist_stack[iter],
                                                   surface_normal,
                                                   ray_starts_inside_sphere,
                                                   ...
                                                   next_ray_dir);
            surface_eta_stack[iter]   = ray_tracer_sphere_eta[local_index];
            surface_color_stack[iter] = ray_tracer_sphere_color[local_index];
            reflectance_stack[iter]   = ray_tracer_sphere_reflectance[local_index];
            transparency_stack[iter]  = ray_tracer_sphere_transparency[local_index];
            luminosity_stack[iter]    = ray_tracer_sphere_luminosity[local_index];
        } else if(obj_index >= plane_start_index && obj_index < plane_stop_index) { // ray hits plane
            int local_index = obj_index - plane_start_index;
            dist_stack[iter] = ray_plane_next_ray(ray_origin,
                                                  ray_dir,
                                                  dist_stack[iter],
                                                  surface_point,
                                                  surface_normal,
                                                  ...
                                                  next_ray_dir);
            surface_eta_stack[iter]   = ray_tracer_plane_eta[local_index];
            surface_color_stack[iter] = ray_tracer_plane_color[local_index];
            reflectance_stack[iter]   = ray_tracer_plane_reflectance[local_index];
            transparency_stack[iter]  = ray_tracer_plane_transparency[local_index];
            luminosity_stack[iter]    = ray_tracer_plane_luminosity[local_index];
            ...
            dist_stack[iter] = ray_plane_next_ray(ray_origin,
                                                  ...
                                                  next_ray_dir);
            surface_eta_stack[iter]   = ray_tracer_box_eta[local_index];
            surface_color_stack[iter] = ray_tracer_box_color[local_index];
            reflectance_stack[iter]   = ray_tracer_box_reflectance[local_index];
            transparency_stack[iter]  = ray_tracer_box_transparency[local_index];
            luminosity_stack[iter]    = ray_tracer_box_luminosity[local_index];
        } else if(dist_stack[iter] == BIG_NUMBER) { // ray hits nothing
            continue;
        } else { // unhandled situation
            gl_FragColor = vec4(1, 0, 0, 0);
            return;
        }
        vec3 next_ray_origin = ray_origin + ray_dir * dist_stack[iter];
        if(ray_tracer_render_mode == 2) {
            normal_projection_stack[iter] = dot(surface_normal, normalize(ray_origin - next_ray_origin));
            if(dist_stack[iter] != BIG_NUMBER && // ray hits something
               luminosity_stack[iter] == 0)      // non-glowing material
            {
                if(abs(surface_eta_stack[iter]) == BIG_NUMBER || // opaque material
                   normal_projection_stack[iter] >= 0)           // frontface
                {
                    phong_stack[iter] = ray_phong(ray_origin,
                                                  next_ray_origin,
                                                  surface_normal);
                } else { // backface
                    phong_stack[iter] = ray_phong(ray_origin,
                ...
                phong_stack[iter] = vec3(0);
            }
        }
        ray_origin       = next_ray_origin;
        ray_dir          = next_ray_dir;
        ignore_obj_index = obj_index;
        //if(ray_tracer_render_mode != 2) {
    ...
    if(ray_tracer_render_mode == 2) {
        ...
        for(int i = last_ray_tracer_bounce_index; i >= 0; i--) { // calculate color forward from skybox to camera
            if(dist_stack[i] == BIG_NUMBER) { // ray hits nothing
                sample_env_map(ray_dir_stack[i], env_map_texture, accum_color); // sample color from direction in skybox
            } else { // ray hits something
                if(luminosity_stack[i] > 0) { // glowing material
                    accum_color = surface_color_stack[i] * luminosity_stack[i];
                } else { // non-glowing material
                    if(abs(surface_eta_stack[i]) == BIG_NUMBER) { // opaque material
                        accum_color = mix(surface_color_stack[i], accum_color + phong_stack[i], reflectance_stack[i]);
                    } else { // transparent material
                        if(normal_projection_stack[i] < 0) { // backface
                            accum_color = mix(accum_color, surface_color_stack[i], clamp(dist_stack[i] / FOG_DISTANCE, 0, 1) * transparency_stack[i]);
                        } else { // frontface
                            accum_color += phong_stack[i] * reflectance_stack[i];
                        }
                    }
                }
            }
